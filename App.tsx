...
import PurchaseProfitCalculator from './components/PurchaseProfitCalculator';
...
const App: React.FC = () => {
  ...
  const useSyncedState = <T,>(tableName: string, initialValue: T, customDbId?: string): [T, React.Dispatch<React.SetStateAction<T>>] => {
        ...
        try {
            ...
            channel.onmessage = (event) => {
                if (event.data && event.data.type === 'UPDATE') {
                    ...
                    remoteDataRef.current = event.data.payload;
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                if (!isMounted) return;
                // If cloud data has already been received, do NOT overwrite it with older local data
                if (hasSyncedWithCloud.current) return;
                
                if (data !== null && data !== undefined) {
                    // Mark as remote update so it doesn't trigger a cloud write just because we loaded it locally
    ...
    useEffect(() => {
        ...
        // If it was a remote update, clear the ref so subsequent local changes are detected
        if (isRemote) {
  ...
  const renderedView = useMemo(() => {
    switch (currentView) {
      ...
      case 'purchaseProfitCalculator': return <PurchaseProfitCalculator purchaseInvoices={purchaseInvoices} items={items} suppliers={suppliers} companyData={companyData} />;
      ...
      case 'generalReport': return <GeneralReport salesInvoices={salesInvoices} salesReturns={salesReturns} purchaseInvoices={purchaseInvoices} purchaseReturns={purchaseReturns} expenses={expenses} customerReceipts={customerReceipts} supplierPayments={supplierPayments} treasuryTransfers={treasuryTransfers} warehouseTransfers={warehouseTransfers} items={items} warehouses={warehouses} treasuries={treasuries} expenseCategories={expenseCategories} customers={customers} suppliers={suppliers} companyData={companyData} defaultValues={defaultValues} />;
