...
const CustomerManager: React.FC<CustomerManagerProps> = ({ customers, setCustomers, showNotification, currentUser, salesInvoices, salesReturns, customerReceipts }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEditing && formData.id) {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `تم تعديل بيانات العميل: ${formData.name}` }));
        } else {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `تم إضافة عميل جديد: ${formData.name}` }));
        }
        resetForm();
    };
    ...
    const performDelete = () => {
        if (customerToDelete) {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `تم حذف العميل: ${customerToDelete.name}` }));
        }
        cancelDelete();
    };
