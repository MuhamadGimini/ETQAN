...
import { ConfirmationModal, DownloadIcon, UploadIcon, Modal, EditIcon, DeleteIcon, DropboxIcon, GoogleDriveIcon, OneDriveIcon } from './Shared';
...
const BackupSettings: React.FC<BackupSettingsProps> = ({ 
    ...
    // State for Cloud Auth & Disconnect Modal
    ...
    const [providerToDisconnect, setProviderToDisconnect] = useState<'dropbox' | 'googleDrive' | 'oneDrive' | null>(null);

    // State for Upload Modal
    const [isUploadModalOpen, setIsUploadModalOpen] = useState(false);
    const [providerToUpload, setProviderToUpload] = useState<string | null>(null);
    const [isUploading, setIsUploading] = useState(false);

    // Fix: Added optional chaining to find call
    ...
    useEffect(() => {
        ...
        const storedCloud = localStorage.getItem('pos_cloud_auth');
        if (storedCloud) setCloudStatus(JSON.parse(storedCloud));
    }, []);

    const updateCloudStatus = (provider: keyof typeof cloudStatus, status: boolean) => {
        const newStatus = { ...cloudStatus, [provider]: status };
        setCloudStatus(newStatus);
        localStorage.setItem('pos_cloud_auth', JSON.stringify(newStatus));
    };

    const handleCloudConnect = (provider: 'dropbox' | 'googleDrive' | 'oneDrive') => {
        // Simulate OAuth flow
        const width = 500;
        const height = 600;
        ...
        if(w) {
            const providerName = provider === 'dropbox' ? 'Dropbox' : provider === 'googleDrive' ? 'Google Drive' : 'OneDrive';
            w.document.write(`<html><body style="background:#f0f0f0;display:flex;justify-content:center;align-items:center;height:100%;font-family:sans-serif;flex-direction:column;"><h2>جاري الاتصال بـ ${providerName}...</h2><p>يرجى الانتظار</p><script>setTimeout(()=>{window.close()}, 2000)</script></body></html>`);
            
            const timer = setInterval(() => {
                if (w.closed) {
                    clearInterval(timer);
                    updateCloudStatus(provider, true);
                    showNotification('save');
                    alert(`تم الربط بنجاح مع ${providerName}!`);
                }
            }, 500);
        }
    };

    const handleRequestDisconnect = (provider: 'dropbox' | 'googleDrive' | 'oneDrive') => {
    ...
    const confirmDisconnect = () => {
        if (providerToDisconnect) {
            updateCloudStatus(providerToDisconnect, false);
    ...
    return (
        <>
        {/* Disconnect Modal */}
        {isDisconnectModalOpen && providerToDisconnect && (
            <ConfirmationModal
                title="تأكيد إلغاء الربط"
                message={`هل أنت متأكد من أنك تريد إلغاء ربط حساب ${providerToDisconnect === 'dropbox' ? 'Dropbox' : providerToDisconnect === 'googleDrive' ? 'Google Drive' : 'OneDrive'}؟ ستتوقف المزامنة السحابية لهذا المزود.`}
        ...
        {/* Upload Modal */}
        {isUploadModalOpen && providerToUpload && (
            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[110] print:hidden">
                <div className="bg-gray-100 dark:bg-gray-800 rounded-xl shadow-2xl p-6 w-full max-w-md border border-white/30 dark:border-gray-600 text-center animate-fade-in-up">
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-4">
                        {isUploading ? 'جاري الرفع...' : 'تأكيد الرفع السحابي'}
                    </h2>
                    
                    {isUploading ? (
                        <div className="flex flex-col items-center justify-center py-6">
                            <div className="animate-spin rounded-full h-16 w-16 border-t-4 border-blue-500 border-opacity-75 mb-4"></div>
                            <p className="text-gray-600 dark:text-gray-300">يرجى الانتظار، يتم رفع البيانات إلى {providerToUpload}...</p>
                        </div>
                    ) : (
                        <>
                            <p className="text-gray-600 dark:text-gray-300 mb-6">
                                هل أنت متأكد من رفع نسخة احتياطية جديدة إلى {providerToUpload === 'dropbox' ? 'Dropbox' : providerToUpload === 'googleDrive' ? 'Google Drive' : 'OneDrive'}؟
                            </p>
                            <div className="flex justify-center space-x-4 space-x-reverse">
                                <button 
                                    onClick={() => { setIsUploadModalOpen(false); setProviderToUpload(null); }} 
        ...
        <Modal title="إعادة تسمية قاعدة البيانات" show={isRenameDbModalOpen} onClose={() => setIsRenameDbModalOpen(false)}>
         ...
         <div className="space-y-8">
            ...
            <div className={cardClass}>
                <h2 className="text-2xl font-bold text-gray-700 dark:text-gray-300 mb-6">خدمات التخزين السحابي (Cloud Storage)</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {/* Dropbox Card */}
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-blue-200 dark:border-blue-900 shadow-sm flex flex-col items-center text-center">
                        <div className="p-4 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 mb-4">
                            <DropboxIcon className="h-10 w-10" />
                        </div>
                        <h3 className="font-bold text-lg text-gray-800 dark:text-gray-200 mb-1">Dropbox</h3>
                        <p className={`text-sm mb-6 font-bold ${cloudStatus.dropbox ? 'text-green-600' : 'text-gray-500'}`}>
                            {cloudStatus.dropbox ? '● متصل' : '○ غير متصل'}
                        </p>
                        {cloudStatus.dropbox ? (
                            <div className="w-full space-y-2">
                                <button onClick={() => handleCloudUpload('dropbox')} className="w-full bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2">
                                    <UploadIcon className="h-5 w-5 text-white" /> رفع نسخة
                                </button>
                                <button onClick={() => handleRequestDisconnect('dropbox')} className="w-full bg-red-100 text-red-600 font-bold py-2 px-4 rounded-lg hover:bg-red-200 transition-colors">
                                    إلغاء الربط
                                </button>
                            </div>
                        ) : (
                            <button onClick={() => handleCloudConnect('dropbox')} className="w-full bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-bold py-2 px-4 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
                                ربط الحساب
                            </button>
                        )}
                    </div>
                    ...
                    {/* Google Drive Card */}
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-green-200 dark:border-green-900 shadow-sm flex flex-col items-center text-center">
                        <div className="p-4 rounded-full bg-green-50 dark:bg-green-900/30 text-green-600 mb-4">
                            <GoogleDriveIcon className="h-10 w-10" />
                        </div>
                        <h3 className="font-bold text-lg text-gray-800 dark:text-gray-200 mb-1">Google Drive</h3>
                        <p className={`text-sm mb-6 font-bold ${cloudStatus.googleDrive ? 'text-green-600' : 'text-gray-500'}`}>
                            {cloudStatus.googleDrive ? '● متصل' : '○ غير متصل'}
                        </p>
                        {cloudStatus.googleDrive ? (
                            <div className="w-full space-y-2">
                                <button onClick={() => handleCloudUpload('googleDrive')} className="w-full bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700 transition-colors flex items-center justify-center gap-2">
                                    <UploadIcon className="h-5 w-5 text-white" /> رفع نسخة
                                </button>
                                <button onClick={() => handleRequestDisconnect('googleDrive')} className="w-full bg-red-100 text-red-600 font-bold py-2 px-4 rounded-lg hover:bg-red-200 transition-colors">
                                    إلغاء الربط
                                </button>
                            </div>
                        ) : (
                            <button onClick={() => handleCloudConnect('googleDrive')} className="w-full bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-bold py-2 px-4 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
                                ربط الحساب
                            </button>
                        )}
                    </div>
                    ...
                    {/* OneDrive Card */}
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-cyan-200 dark:border-cyan-900 shadow-sm flex flex-col items-center text-center">
                        <div className="p-4 rounded-full bg-cyan-50 dark:bg-cyan-900/30 text-cyan-600 mb-4">
                            <OneDriveIcon className="h-10 w-10" />
                        </div>
                        <h3 className="font-bold text-lg text-gray-800 dark:text-gray-200 mb-1">OneDrive</h3>
                        <p className={`text-sm mb-6 font-bold ${cloudStatus.oneDrive ? 'text-green-600' : 'text-gray-500'}`}>
                            {cloudStatus.oneDrive ? '● متصل' : '○ غير متصل'}
                        </p>
                        {cloudStatus.oneDrive ? (
                            <div className="w-full space-y-2">
                                <button onClick={() => handleCloudUpload('oneDrive')} className="w-full bg-cyan-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-cyan-700 transition-colors flex items-center justify-center gap-2">
                                    <UploadIcon className="h-5 w-5 text-white" /> رفع نسخة
                                </button>
                                <button onClick={() => handleRequestDisconnect('oneDrive')} className="w-full bg-red-100 text-red-600 font-bold py-2 px-4 rounded-lg hover:bg-red-200 transition-colors">
                                    إلغاء الربط
                                </button>
                            </div>
                        ) : (
                            <button onClick={() => handleCloudConnect('oneDrive')} className="w-full bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-bold py-2 px-4 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
                                ربط الحساب
                            </button>
                        )}
                    </div>
                    ...
                    {databases?.map(db => (
                        <div key={db.id} className="flex items-center justify-between p-2 bg-black/5 dark:bg-white/5 rounded-lg">
                            ...
                            <div className="flex space-x-1 space-x-reverse">
                                <button onClick={() => handleSwitchDatabase(db.id)} disabled={db.id === activeDatabaseId} className="bg-green-50 text-white text-sm font-bold py-1 px-3 rounded disabled:bg-gray-400 disabled:cursor-not-allowed hover:bg-green-600">تفعيل</button>
                                ...
                                <button onClick={() => { setDbToDelete(db); setIsDeleteDbModalOpen(true); }} disabled={db.id === activeDatabaseId || db.id === 'default' || (databases && databases.length <= 1)} className="p-2 text-red-600 disabled:text-gray-400 disabled:cursor-not-allowed hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full"><DeleteIcon /></button>
                            </div>
                        </div>
                    ))}
                </div>
