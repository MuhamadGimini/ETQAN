...
interface CustomerReceiptManagerProps {
...
const CustomerReceiptManager: React.FC<CustomerReceiptManagerProps> = ({
    ...
    const calculateTreasuryBalance = (treasuryId: number) => {
        ...
        customerReceipts.forEach(rec => {
            if (rec.treasuryId === treasuryId && (rec.paymentMethod === 'cash' || rec.paymentMethod === 'check') && rec.id !== formData.id) {
                balance += rec.amount;
            }
        });
        supplierPayments.forEach(p => {
            if (p.treasuryId === treasuryId && (p.paymentMethod === 'cash' || p.paymentMethod === 'check')) balance -= p.amount;
        });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEditing && formData.id) {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بتعديل سند قبض رقم ${formData.id} من العميل ${customers.find(c => c.id === formData.customerId)?.name || ''} بمبلغ [SENSITIVE:${formData.amount}]` }));
        } else {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بإنشاء سند قبض رقم ${newReceipt.id} من العميل ${customers.find(c => c.id === formData.customerId)?.name || ''} بمبلغ [SENSITIVE:${formData.amount}]` }));
        }
        resetForm();
    };
...
export default CustomerReceiptManager;