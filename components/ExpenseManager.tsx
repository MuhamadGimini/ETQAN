...
const ExpenseManager: React.FC<ExpenseManagerProps> = ({ 
    ...
    const calculateTreasuryBalance = (treasuryId: number) => {
        ...
        expenses.forEach(exp => {
            if(exp.treasuryId === treasuryId && exp.id !== formData.id){
                balance -= exp.amount;
            }
        });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEditing && formData.id) {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `تم تعديل مصروف رقم ${formData.id} بقيمة [SENSITIVE:${formData.amount}]` }));
        } else {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `تم إضافة مصروف جديد رقم ${newExpense.id} بقيمة [SENSITIVE:${newExpense.amount}]` }));
        }
        resetForm();
    };
    ...
    const performDelete = () => { 
        if (expenseToDelete) { 
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `تم حذف مصروف رقم ${expenseToDelete.id} بقيمة [SENSITIVE:${expenseToDelete.amount}]` }));
        } 
        cancelDelete(); 
    };
