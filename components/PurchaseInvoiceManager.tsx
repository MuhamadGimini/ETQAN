...
const PurchaseInvoiceManager: React.FC<PurchaseInvoiceManagerProps> = ({ 
    purchaseInvoices, setPurchaseInvoices, heldPurchaseInvoices, setHeldPurchaseInvoices, purchaseReturns, supplierPayments, items, setItems, suppliers, setSuppliers,
    ...
    const calculateTreasuryBalance = (treasuryId: number) => {
        ...
        customerReceipts.forEach(rec => { if (rec.treasuryId === treasuryId && ['cash','check'].includes(rec.paymentMethod)) balance += rec.amount; });
        supplierPayments.forEach(p => { if (p.treasuryId === treasuryId && ['cash','check'].includes(p.paymentMethod)) balance -= p.amount; });
        ...
        if (treasuryId === defaultValues.defaultTreasuryId) {
            ...
            purchaseInvoices.forEach(inv => { if (inv.paidAmount && String(inv.id) !== String(newInvoice.id)) balance -= inv.paidAmount; });
    ...
    const handleSaveInvoice = (printAfterSave: boolean = false) => {
        ...
        if (isEditing) {
            const updated = { ...newInvoice, id: finalizedId, lastModifiedBy: currentUser.username, lastModifiedAt: new Date().toISOString() };
            ...
            const itemsTotal = updated.items.reduce((sum, item) => sum + (item.quantity * item.price), 0);
            const totalAmount = (itemsTotal - updated.discount) * (1 + updated.tax / 100);
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بتعديل فاتورة مشتريات رقم ${finalizedId} للمورد ${suppliers.find(s => s.id === newInvoice.supplierId)?.name || ''} بقيمة إجمالية [SENSITIVE:${totalAmount.toFixed(2)}]` }));
        } else {
            const created = { ...newInvoice, id: finalizedId, createdBy: currentUser.username, createdAt: new Date().toISOString() };
            setPurchaseInvoices(prev => [...prev, created]);
            showNotification('add'); if (printAfterSave) handlePrint(created);
            const itemsTotal = created.items.reduce((sum, item) => sum + (item.quantity * item.price), 0);
            const totalAmount = (itemsTotal - created.discount) * (1 + created.tax / 100);
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بإنشاء فاتورة مشتريات رقم ${finalizedId} للمورد ${suppliers.find(s => s.id === newInvoice.supplierId)?.name || ''} بقيمة إجمالية [SENSITIVE:${totalAmount.toFixed(2)}]` }));
        }
        resetForm();
    };
                             ...
                             <div className="lg:col-span-2 relative">
                                ...
                                {newInvoice.type === 'cash' && (
                                     <div className="absolute top-full right-0 text-xl font-black mt-1 whitespace-nowrap z-0">
                                        ...
                                        <span className="text-blue-600"><FormattedNumber value={calculateTreasuryBalance(defaultValues.defaultTreasuryId)} /></span>
                                     </div>
                                )}
                             </div>
                        </div>
