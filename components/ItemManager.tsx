...
const ItemManager: React.FC<ItemManagerProps> = ({ 
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEditing && formData.id) {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `تم تعديل بيانات الصنف: ${formData.name}` }));
        } else {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `تم إضافة صنف جديد: ${formData.name}` }));
        }
        handleCloseModal();
    };
    ...
    const performDelete = () => {
        if (itemToDelete) {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `تم حذف الصنف: ${itemToDelete.name}` }));
        }
        resetDeleteProcess();
    };
