...
const AllSuppliersStatement: React.FC<AllSuppliersStatementProps> = ({
    ...
    const handlePrint = () => {
        ...
        // Use filtered summaries to respect the zero balance filter
        const printableSummaries = filteredSummaries;

        // Recalculate totals based on the filtered list for consistency.
        ...
        const reportHtml = `
            ...
            <body class="p-8" onload="window.print(); window.close();">
                ...
                <h2 class="text-2xl font-bold text-center mb-4">تقرير أرصدة الموردين النهائية</h2>
                
                <div class="grid grid-cols-3 gap-4 mb-6 text-sm border p-4 rounded-lg bg-gray-50">
                    <div>عدد الموردين: <span class="font-bold">${printableSummaries.length}</span></div>
                    <div>إجمالي المشتريات: <span class="font-bold">${totals.totalPurchases.toFixed(2)}</span></div>
                ...
                <table class="w-full text-right border-collapse border border-gray-400">
                    <thead class="bg-gray-200">
                        <tr>
                             ...
                             <th class="p-2 border border-gray-300">اسم المورد</th>
                    ...
                    <tbody>
                        ${printableSummaries.map((summary, index) => `
                            <tr class="border-b">
                                ...
                                <td class="p-2 border border-gray-300 font-bold ${summary.closingBalance > 0 ? 'text-red-700' : 'text-green-700'}">${summary.closingBalance.toFixed(2)}</td>
                            </tr>
                        `).join('')}
                        <tr class="bg-gray-100 font-bold">
                            <td class="p-2 border border-gray-300 text-center" colspan="2">إجمالي الدائنية (لهم)</td>
                            <td class="p-2 border border-gray-300 text-red-800">${totalPayables.toFixed(2)}</td>
                        </tr>
                        <tr class="bg-gray-100 font-bold">
                            <td class="p-2 border border-gray-300 text-center" colspan="2">إجمالي المديونية (عليهم)</td>
