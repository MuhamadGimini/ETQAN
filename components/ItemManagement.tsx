        ...
        <Modal title={isViewing ? 'عرض بيانات الصنف' : isEditing ? 'تعديل بيانات الصنف' : 'إضافة صنف جديد'} show={isModalOpen} onClose={handleCloseModal}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="relative"><label className={labelClass}>الوحدة {requiredSpan}</label><input type="text" value={unitSearchQuery} onChange={handleUnitSearchChange} onFocus={() => setIsUnitSuggestionsOpen(true)} onBlur={() => { setTimeout(() => { if (isUnitSuggestionsOpen && suggestedUnits.length > 0 && unitSearchQuery) { handleUnitSelect(suggestedUnits[0]); } setIsUnitSuggestionsOpen(false); }, 250); }} className={inputClass} required disabled={isViewing} autoComplete="off" />{isUnitSuggestionsOpen && <ul className="absolute z-60 w-full bg-white dark:bg-gray-800 border rounded mt-1 max-h-40 overflow-y-auto shadow-lg">{suggestedUnits.map(u => <li key={u.id} onMouseDown={() => handleUnitSelect(u)} className="p-2 hover:bg-gray-100 cursor-pointer">{u.name}</li>)}</ul>}</div>
                    <div className="relative"><label className={labelClass}>المخزن {requiredSpan}</label><input type="text" value={warehouseSearchQuery} onChange={handleWarehouseSearchChange} onFocus={() => setIsWarehouseSuggestionsOpen(true)} onBlur={() => { setTimeout(() => { if (isWarehouseSuggestionsOpen && suggestedWarehouses.length > 0 && warehouseSearchQuery) { handleWarehouseSelect(suggestedWarehouses[0]); } setIsWarehouseSuggestionsOpen(false); }, 250); }} className={inputClass} required disabled={isViewing} autoComplete="off" />{isWarehouseSuggestionsOpen && <ul className="absolute z-60 w-full bg-white dark:bg-gray-800 border rounded mt-1 max-h-40 overflow-y-auto shadow-lg">{suggestedWarehouses.map(w => <li key={w.id} onMouseDown={() => handleWarehouseSelect(w)} className="p-2 hover:bg-gray-100 cursor-pointer">{w.name}</li>)}</ul>}</div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <input type="text" placeholder="ابحث بالاسم أو الباركود..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`${inputClass} w-64`} />
                         <div className="relative w-48">
                            <input type="text" value={warehouseFilterSearchQuery} onChange={handleWarehouseFilterSearchChange} onFocus={() => setIsWarehouseFilterSuggestionsOpen(true)} onBlur={() => {
                                setTimeout(() => {
                                    if (isWarehouseFilterSuggestionsOpen && suggestedFilterWarehouses.length > 0 && warehouseFilterSearchQuery) {
                                        handleWarehouseFilterSelect(suggestedFilterWarehouses[0]);
                                    }
                                    setIsWarehouseFilterSuggestionsOpen(false);
                                }, 250);
                            }} placeholder="فلتر المخزن..." className={inputClass} autoComplete="off" />
