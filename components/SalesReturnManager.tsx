...
const SalesReturnManagement: React.FC<SalesReturnManagementProps> = ({ 
    ...
    expenses, treasuryTransfers, treasuries, supplierPayments
}) => {
    ...
    const handleSaveReturn = (printAfterSave: boolean = false) => {
        ...
        try {
            ...
            if (isEditing) { const updatedReturn = { ...newReturn, lastModifiedBy: currentUser.username, lastModifiedAt: new Date().toISOString() }; setSalesReturns(prev => prev.map(inv => inv.id === newReturn.id ? updatedReturn : inv)); showNotification('edit'); if (printAfterSave) handlePrint(updatedReturn); window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بتعديل مرتجع مبيعات رقم ${newReturn.id} للعميل ${customers.find(c => c.id === newReturn.customerId)?.name || ''} بقيمة إجمالية [SENSITIVE:${total.toFixed(2)}]` })); }
            else { const createdReturn = { ...newReturn, createdBy: currentUser.username, createdAt: new Date().toISOString() }; setSalesReturns(prev => [...prev, createdReturn]); showNotification('add'); if (printAfterSave) handlePrint(createdReturn); window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بإنشاء مرتجع مبيعات رقم ${createdReturn.id} للعميل ${customers.find(c => c.id === newReturn.customerId)?.name || ''} بقيمة إجمالية [SENSITIVE:${total.toFixed(2)}]` })); }
            resetForm();
