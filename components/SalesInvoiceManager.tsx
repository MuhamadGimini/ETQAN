...
const SalesInvoiceManager: React.FC<SalesInvoiceManagerProps> = ({ 
    salesInvoices, setSalesInvoices, heldInvoices, setHeldInvoices, salesReturns, customerReceipts, items, setItems, customers, setCustomers,
    ...
    const performActualSave = (printAfterSave: boolean) => {
        ...
        if (isEditing) {
            const finalizedId = typeof newInvoice.id === 'string' ? getNextInvoiceId() : newInvoice.id;
            const updated = { ...newInvoice, id: finalizedId, lastModifiedBy: currentUser.username, lastModifiedAt: new Date().toISOString() };
            setSalesInvoices(prev => prev.map(inv => inv.id === newInvoice.id ? updated : inv));
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بتعديل فاتورة مبيعات رقم ${finalizedId} للعميل ${customers.find(c => c.id === newInvoice.customerId)?.name || ''} بقيمة إجمالية [SENSITIVE:${totalAmount.toFixed(2)}]` }));
        } else {
            const created = { ...newInvoice, id: getNextInvoiceId(), createdBy: currentUser.username, createdAt: new Date().toISOString() };
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بإنشاء فاتورة مبيعات رقم ${created.id} للعميل ${customers.find(c => c.id === newInvoice.customerId)?.name || ''} بقيمة إجمالية [SENSITIVE:${totalAmount.toFixed(2)}]` }));
        }
        resetForm();
...
export default SalesInvoiceManager;
