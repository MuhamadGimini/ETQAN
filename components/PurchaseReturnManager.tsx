...
const PurchaseReturnManagement: React.FC<PurchaseReturnManagementProps> = ({ 
    ...
    customerReceipts, expenses, treasuryTransfers, treasuries
}) => {
    ...
    const handleSaveReturn = (printAfterSave: boolean = false) => {
        ...
        try {
            ...
            if (isEditing) { const updatedReturn = { ...newReturn, lastModifiedBy: currentUser.username, lastModifiedAt: new Date().toISOString() }; setPurchaseReturns(prev => prev.map(inv => inv.id === newReturn.id ? updatedReturn : inv)); showNotification('edit'); if (printAfterSave) handlePrint(updatedReturn); window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بتعديل مرتجع مشتريات رقم ${newReturn.id} للمورد ${suppliers.find(s => s.id === newReturn.supplierId)?.name || ''} بقيمة إجمالية [SENSITIVE:${total.toFixed(2)}]` })); }
            else { const createdReturn = { ...newReturn, createdBy: currentUser.username, createdAt: new Date().toISOString() }; setPurchaseReturns(prev => [...prev, createdReturn]); showNotification('add'); if (printAfterSave) handlePrint(createdReturn); window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بإنشاء مرتجع مشتريات رقم ${createdReturn.id} للمورد ${suppliers.find(s => s.id === newReturn.supplierId)?.name || ''} بقيمة إجمالية [SENSITIVE:${total.toFixed(2)}]` })); }
            resetForm();
