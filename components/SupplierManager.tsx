...
const SupplierManagement: React.FC<SupplierManagementProps> = ({ suppliers, setSuppliers, showNotification, currentUser, purchaseInvoices, purchaseReturns, supplierPayments }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEditing && formData.id) {
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بإضافة مورد جديد ${formData.name}` }));
        }
        resetForm();
    };
    ...
    const performDelete = () => { 
        if (supplierToDelete) { 
            ...
            window.dispatchEvent(new CustomEvent('logTransaction', { detail: `قام المستخدم ${currentUser.fullName} بحذف المورد ${supplierToDelete.name}` }));
        } 
        cancelDelete(); 
    };
